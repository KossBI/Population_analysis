    ...
    if D > 9:
        if D < 25:
            ...
            LCL = SCR + ((((df['Стандартный вес группы'].pow(2) * df['Коэффициент смертности'] / df['Численность населения']).sum() /
                         ...
                         * (stats.chi2.ppf(1 - alpha/2, 2 * df['Число умерших'].sum())/2 - df['Число умерших'].sum()) * 100000)
            UCL = SCR + ((((df['Стандартный вес группы'].pow(2) * df['Коэффициент смертности'] / df['Численность населения']).sum() /
                           df['Число умерших'].sum()) ** 0.5)
                         * (stats.chi2.ppf(alpha/2, 2 * df['Число умерших'].sum() + 2)/2 - df['Число умерших'].sum()) * 100000)
        else:
...
# Короткий вывод:
# 1) В Районе 3 женская смертность равна мужской.
...
# Короткий вывод:
# 1) Район 1 и Район 2 мало отличаются по структуре смертности (смертность от причин отличных от болезней сердца выше в Районе 2).
# 2) В Районе 3 женская смертность и смертность от болезней сердца ниже, чем в Районах 1 и 2.
# При этом смертность от причин отличных от болезней сердца выше в Районе 3, чем в районе 2,
...
# Короткий вывод:
# 1) В районах 1 и 2 смертность от болезней сердца превышает смертность от остальных причин.
# 2) Районах 1 и 2 женская смертность не отличается от мужской.
# 3) В Районе 3 женская смертность ниже мужской.
# 4) в Районе 3 смертность от болезней сердца выше, чем от остальных причин.
