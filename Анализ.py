...
def N_SCR_SE_calc(
    ...
    """
    Рассчитывает чуммарную численность населения, стандартизованный коэффициент смертности и стандартную ошибку для него
    по датафрейму формата исходных данных.
    df: датафрейм формата исходных данных
    Возвращает суммарную численность населения, стандартизованный коэффициент смертности и его стандартную ошибку.
    """
    ...
    # Вычисление стандартной ошибки стандартизованного коэффициента смертности на 100000 человек для среза (пуассоновская аппроксимация).
    SE = ((df['Стандартный вес группы'].pow(2) * df['Коэффициент смертности'] / df['Численность населения']).sum() ** 0.5) * 100000
    return N.item(), SCR.item(), SE.item()


# In[9]:


def N_SCR_SE_by_slice(
    df: pd.DataFrame,
    district: str,
    year: int,
    target: str = None):
    ...
    """
    Расчитывает суммарную численность населения, стандартизованный коэффициент смертности и его стандартную ошибку
    посредством функций get_slice и N_SCR_SE_calc.
    df: исходный датафрейм
    district: муниципалитет
    year: год
    target: пол, причина смерти или None
    Возвращает суммарную численность населения, стандартизованный коэффициент смертности и стандартную ошибку.
    """
    
    # Создание таблицы среза через вызов функции get_slice.
    df_slice = get_slice(df, district, year, target)
    return N_SCR_SE_calc(df_slice)
...
columns_list = ['Муниципалитет', 'Год', 'Пол', 'Причина смерти', 'Численность населения', 'СКС', 'Стандартная ошибка']
...
# Расчёт значений суммарной численности населения, СКС и стандартной ошибки,
# и включение этих данных в содержимое листа result_district_year_list
result_district_year_list = [row + ['Все', 'Все'] + list(N_SCR_SE_by_slice(df, row[0], row[1])) \
...
result_district_year_gender_list = [row + ['Все'] + list(N_SCR_SE_by_slice(df, row[0], row[1], row[2])) \
...
result_district_year_death_cause_list = [[row[0], row[1], 'Все', row[2]] + list(N_SCR_SE_by_slice(df, row[0], row[1], row[2])) \
...
df_export = df_result[['Муниципалитет', 'Год', 'Пол', 'Причина смерти', 'СКС', 'Стандартная ошибка']].copy()
# Сохранение в CSV.
df_export.to_csv('СКС_результаты.csv', index=False, encoding='utf-8-sig')

# Округление до целых.
df_export[['СКС', 'Стандартная ошибка']] = df_export[['СКС', 'Стандартная ошибка']].round(0).astype(int)
# Сохранение в CSV.
